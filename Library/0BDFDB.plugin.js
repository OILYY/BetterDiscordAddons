	...
	return class BDFDB_Frame {
		...
		load () {
				...
				if (changeLogEntries.length) BDFDB.ModalUtils.open(plugin, {
					...
					footerChildren: (plugin == BDFDB || plugin == this || PluginStores.loaded[plugin.name] && PluginStores.loaded[plugin.name] == plugin && plugin.author == "Oily") && BDFDB.ReactUtils.createElement("div", {
						...
						children: [{
							...
							href: "https://www.patreon.com/OilyGranySmith",
							name: "Patreon",
							icon: "PATREON"
						}, {
			...
			const requestLibraryHashes = tryAgain => {
				requestFunction("https://api.github.com/repos/OILYY/BetterDiscordAddons/contents/Library/_res/", {timeout: 60000}, (e, r, b) => {
			...
			const requestLibraryData = tryAgain => {
				const parseCSS = css => {
					...
					if (backupObj.backup && backupObj.hashIsSame || UserStore && UserStore.getCurrentUser().id == "731431244235145216") parseData(backupObj.backup);
					else requestFunction(`https://OILYY.github.io/BetterDiscordAddons/Library/_res/${dataFileName}`, {timeout: 60000}, (e, r, b) => {
				...
				else requestFunction(`https://OILYY.github.io/BetterDiscordAddons/Library/_res/${cssFileName}`, {timeout: 60000}, (e, r, b) => {
					if ((e || !b || r.statusCode != 200) && tryAgain) return BDFDB.TimeUtils.timeout(_ => requestLibraryData(), 10000);
					if (!e && b && r.statusCode == 200) {
						if (backupObj.backup && backupObj.backup.replace(/\s/g, "") == b.replace(/\s/g, "")) {
				...
				Internal.getPluginURL = function (plugin) {
					...
					if (BDFDB.ObjectUtils.is(plugin)) {
						if (InternalData.PluginUrlMap && InternalData.PluginUrlMap[plugin.name]) return InternalData.PluginUrlMap[plugin.name];
							...
							return `https://OILYY.github.io/BetterDiscordAddons/Plugins/${name}/${name}.plugin.js`;
						}
					}
					else return "";
				};
				...
				Internal._processAvatarMount = function (user, avatar, wrapper) {
					...
					if (Node.prototype.isPrototypeOf(avatar) && (avatar.className || "").indexOf(BDFDB.disCN.bdfdbbadgeavatar) == -1) {
						...
						if (BDFDB_Patrons[user.id] && BDFDB_Patrons[user.id].active) {
							link = "https://www.patreon.com/OilyGranySmith";
